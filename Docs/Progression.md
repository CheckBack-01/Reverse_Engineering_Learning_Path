...
**PBR closed (F1+):** **0** · **PAD closed (F1+):** **0** · **F0 mini-PBR closed:** **0**
**Note:** PBR/PAD windows for F1+ are starting; **F0** is an active, short, non-daily micro-track running in parallel with F1.
...
> Tip: when you close a PBR/PAD, increment counters and link the evidence + PAD report. For F0, track mini-PBRs and drill accuracy/time.
...
| Track | Title                      | Mini-PBR | PAD | Status        | Notes                         |
| ----- | -------------------------- | -------: | --: | ------------- | ----------------------------- |
...
| 4B01  | x86\_64 / ABI / ASM                         |   0 |   0 | ☐ Not started |       |
| 5B01  | Linking/Loading (ELF, PLT/GOT, LD\_PRELOAD) |   0 |   0 | ☐ Not started |       |
...
*All items remain ☐ Not started for PBR/PAD; only light reinforcement.*
...
1. Change status: `☐` → `◐` → `✔`.
2. F1+ PBR/PAD; **F0 mini-PBR** 
3. Add links:

   * **Labs (F1+):** `phases/F1_ENIAC/0B01/D14_FHS/`
   * **Evidence (F1+):** `phases/F1_ENIAC/0B01/D14_FHS/evidence/`
   * **PAD (F1+):** `phases/F1_ENIAC/0B01/PAD/`
...
* **F0** is a **short, non-daily** micro-track to accelerate F1 comprehension; after \~6 weeks, keep light refreshers inside BackLogs.
